...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x64bfCBe4480B53E8234Ca258a96720F29fe6A6fB) // DollarRewardPool
    ];
    ...
    function initialize(
        ...
        allocateSeigniorageSalary = 0.01 ether;

        // First 21 epochs with 1.5% expansion
        bootstrapEpochs = 21;
    ...
    function _calculateMaxSupplyExpansionPercent(uint256 _dollarSupply) internal returns (uint256) {
        if (_dollarSupply >= dollarSupplyTarget) {
            ...
            maxSupplyExpansionPercent = maxSupplyExpansionPercent.mul(9700).div(10000); // -3%
            if (maxSupplyExpansionPercent < 100) {
                maxSupplyExpansionPercent = 100; // min 1%
            }
        }
    ...
    function getDollarExpansionRate() public view returns (uint256 _rate) {
        if (epoch < bootstrapEpochs) { // 21 first epochs with 1.5% expansion
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            // 21 first epochs with 1.5% expansion
            ...
            if (previousEpochDollarPrice >= dollarPriceCeiling) {
                IBoardroom(boardroom).setWithdrawFee(1);
                // Expansion ($DOLLAR Price > 1 $METF): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bond).totalSupply();
