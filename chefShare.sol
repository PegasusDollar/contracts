...
contract PShareRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public pSharePerSecond = 0.13888888888 ether; // 12,000,000 pshare / (1000DAY)
    ...
    constructor(address _pshare, uint256 _poolStartTime) public {
        ...
        pSharePerSecond = TOTAL_REWARDS.div(runningTime);
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "PShareRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOperatorOrTreasury() {
        require(
            ...
            "PShareRewardPool: caller is not the operator/treasury"
        );
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "PShareRewardPool: existing pool?"
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(pSharePerSecond);
            return poolEndTime.sub(_fromTime).mul(pSharePerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(pSharePerSecond);
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _pshareReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accPSharePerShare = accPSharePerShare.add(
                _pshareReward.mul(1e18).div(tokenSupply)
            );
        }
        return
            user.amount.mul(accPSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accPSharePerShare = pool.accPSharePerShare.add(
                _pshareReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                .amount
                .mul(pool.accPSharePerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (_pending > 0) {
                _safePShareTransfer(msg.sender, _pending);
    ...
    function _withdraw(
        ...
        uint256 _pending = user
            ...
            .sub(user.rewardDebt);
        if (_pending > 0) {
            _safePShareTransfer(_account, _pending);
    ...
    function _safePShareTransfer(address _to, uint256 _amount) internal {
        ...
        onlyOperatorOrTreasury
    {
        ...
        uint256 _oldRate = pSharePerSecond;
        massUpdatePools();
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 180 days) {
            // do not allow to drain core token (PShare or lps) if less than 180 days after pool ends
